...
def get_satisfaction_for_users(predictions: pd.DataFrame, aggregated_rankings: pd.DataFrame): 
    """Computes satisfaction for user"""
    movie_ids = aggregated_rankings.columns
    k = len(movie_ids)
    group_satisfactions = pd.DataFrame(columns = ["userId", "satisfaction"]).set_index("userId")
    for index_user, row in predictions.iterrows():
        group_satisfactions.loc[index_user] = sum(row.loc[movie_ids])

    ideal_satisfaction = pd.DataFrame(columns = ["userId", "satisfaction"]).set_index("userId")

    for index_user, row in predictions.iterrows():
        top_k_movies = row.sort_values(ascending=False)[:k] # We assume that there are no NaNs (well synce it is output from Collaborative Filtering there will not be any)
...
def test():
    ...
    predictions = predictions.pivot_table(index="userId", columns="movieId", values="rating")
    ...
    print("DISAGREEMENTS")
    penalties, group_disagreement = get_disagreements_based_on_order(predictions, aggregated_rankings)
